...
export const register = async (req, res, next) => {
  try {
    let { firstName, lastName, username, phone, email, password, city, role } =
      req.body;

    if (!firstName || !lastName || !username || !phone || !password || !city)
    ...
    if (username == process.env.SUPER_ADMIN_USERNAME) role = "super_admin";
    else if (username == process.env.MANAGER_USERNAME) role = "manager";
...
export const login = async (req, res, next) => {
  try {
    ...
    if (!findedUser)
      return next(createError(400, "Wrong Credentials - username"));
    ...
    if (!isPasswordCorrect)
      return next(createError(401, "Wrong Credentials - password"));
