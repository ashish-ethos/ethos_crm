...
export default function marketingReducer(state = initialState, action) {
  switch (action.type) {
    ...
    case DELETE_CAMPAIGN:
      return { ...state, campaigns: state.campaigns.filter(c => (c.id || c._id) !== action.payload) };
    case UPDATE_CAMPAIGN:
      return {
        ...state,
        campaigns: state.campaigns.map(c => (c.id === action.payload.id || c._id === action.payload.id) ? { ...c, ...action.payload.campaign } : c)
      };
    default:
      return state;
  }
